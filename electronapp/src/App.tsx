...
import './App.css';
import dragImage from './sky.png'; // ドラッグ中に表示する画像
...
function App() {
  ...
  const handleDragStart = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    if (
      ...
      return; // 入力フィールドやボタン、黒いブロックをクリックした場合はドラッグモードに移行しない
    }
    setDragging(true);
  ...
  const handleDragEnd = () => {
    setDragging(false);
    setIsMoveMode(false); // 移動モードを終了する
  };
      ...
      >
        ...
        {dragging ? (
          <img
            ...
            className="App-image"
            alt="Example"
            draggable="false" // ドラッグ中はdragImageのみ反応するようにするためにdraggable属性を設定
          />
        ) : (
